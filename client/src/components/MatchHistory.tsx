    ...
    <Card className="stat-card p-5">
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
        {matches.slice(0, displayedMatches).map((match) => (
          <div 
            key={match.id} 
            className={`bg-background rounded p-3 hover:border hover:border-primary transition-colors cursor-pointer ${selectedMatchId === match.id ? 'border border-primary' : ''}`}
            onClick={() => handleMatchSelect(match.id)}
          >
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center">
                <div className="text-primary font-medium text-sm mr-2">#{match.placement}</div>
                <div>
                  <div className="text-sm font-medium">{match.map} - {match.gameMode}</div>
              ...
              <div className="text-right">
                <div className="text-accent font-medium">{match.kills} Kills</div>
                <div className="text-xs text-muted-foreground">{match.damage} Damage</div>
              </div>
            </div>
            <div className="flex items-center text-xs">
              <Badge variant="outline" className="bg-primary/20 text-primary px-2 py-0.5 rounded mr-2">
                {Math.floor(match.duration / 60)} min
              </Badge>
              <Badge variant="outline" className="bg-background/60 px-2 py-0.5 rounded mr-2 border border-muted">
                {match.assists} Assists
              </Badge>
              <Badge variant="outline" className="bg-background/60 px-2 py-0.5 rounded border border-muted">
