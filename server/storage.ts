...
export const storage = {
  ...
  saveUserPreferences: async (
    ...
    if (existing) {
      ...
      return await db.insert(userPreferences).values(validatedData).returning();
    }
  },
  ...
  addSearchHistory: async (data: z.infer<typeof searchHistoryInsertSchema>) => {
  ...
  saveMatchToCache: async (matchId: string, matchData: any) => {
    ...
    if (existing) {
      ...
      return await db.insert(matchCache).values(validatedData).returning();
    }
  },
  ...
  savePlayerToCache: async (
    ...
    if (existing) {
      ...
      return await db.insert(playerCache).values(validatedData).returning();
    }
  },
  ...
  fetchPlayerData: async (platform: string, playerName: string) => {
    try {
      ...
      try {
        ...
        const matchPromises = recentMatchIds.slice(0, 5).map((matchId) =>
          pubgApi
        ...
        matchDetails = (await Promise.all(matchPromises)).filter(Boolean);
      } catch (error) {
        console.error(
    ...
    try {
      ...
      if (typeof matchData === "object" && matchData !== null) {
        ...
        if ("telemetryUrl" in matchData && matchData.telemetryUrl) {
          console.log("Using telemetryUrl directly:", matchData.telemetryUrl);
        ...
        // Then check if we have assets with URLs
          ...
          if (assetWithUrl && "url" in assetWithUrl) {
            console.log("Found telemetry URL in assets:", assetWithUrl.url);
...
function processPlayerStats(
  ...
  const headshotRate =
    combinedStats.kills > 0
      ? ((combinedStats.headshotKills / combinedStats.kills) * 100).toFixed(1) +
        "%"
      : "0%";

  const top10Rate =
    combinedStats.totalGames > 0
      ? ((combinedStats.top10s / combinedStats.totalGames) * 100).toFixed(1) +
        "%"
      : "0%";
  ...
  if (matchDetails && matchDetails.length > 0) {
    ...
    for (const mapName in mapCounts) {
      maps.push({
        ...
        percentage:
          ((mapCounts[mapName] / matchDetails.length) * 100).toFixed(1) + "%",
      });
    }
  ...
  return {
    ...
    participants: participants.map((p: any) => ({
      ...
      teamId: rosters.find((r: any) =>
        r.relationships.participants.data.some((rp: any) => rp.id === p.id),
  ...
  try {
    ...
    const playZoneEvents = telemetryData.filter(
      (event: any) =>
        event &&
        event._T &&
        (event._T === "LogGameStatePeriodic" || event._T === "LogPhaseChange"),
    );
    ...
    return {
      ...
      playZones: playZoneEvents
        .filter(
          (event: any) =>
            event && event.gameState && event.gameState.safetyZonePosition,
        )
        .map((event: any) => ({
          position: {
